function [density] = voronoimaker5(strutthickness,sigma_nucleus,sigma_node,type,partnumber)
...
filename = [partnumber,ending];

filename = join(filename,"");

filename_random = [partnumber, "-randomizer_", ending];
filename_random = join(filename_random,"");
filename_vorseeds = [partnumber,"-vorseed_", ending];
filename_vorseeds = join(filename_vorseeds,"");
filename_struts = [partnumber,"-strutpoints_",ending];
filename_struts = join(filename_struts,"");
filename_randomnode = [partnumber,"-randomnode_",ending];
filename_randomnode = join(filename_randomnode,"");
...
Ny = 10;
extrusiondepth = 30;
threshold = 0; %minimum size of polygons to be added
...
%if it's in test mode, name it test specimen
if ~writedata
    filename = 'test_specimen.stl';
end
axis square
hold on
...
%save all the .csv data
if writedata
csvwrite(filename_random,randomvectors)
csvwrite(filename_vorseeds,Points)
csvwrite(filename_struts,strutconnections)
csvwrite(filename_randomnode,randomvectors_node)
end
